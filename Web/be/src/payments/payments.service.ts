...
export class PaymentsService {
  ...
  async handleWebhook(webhookPayload: PayOSWebhookDto): Promise<void> {
    if (!webhookPayload || !webhookPayload.data || !webhookPayload.signature) {
      this.logger.log('Received a webhook test/verification request.');
      return;
    }

    const { data, signature } = webhookPayload;
    const webhookData: any = data;
    const orderCode = webhookData.orderCode;

    const checksumKey = this.configService.get<string>('PAYOS_CHECKSUM_KEY');
    if (!checksumKey) {
      this.logger.error('CRITICAL: PAYOS_CHECKSUM_KEY is not configured.');
      return;
    }
    ...
    for (const key of sortedKeys) {
      ...
      let value = webhookData[key];

      if (value === null) {
        value = '';
      }

      dataParts.push(`${key}=${String(value)}`);
    }
    ...
    if (generatedSignature !== signature) {
      ...
      // this.logger.debug(`Data string generated by server: ${dataToSign}`);
      // this.logger.debug(`Signature from PayOS: ${signature}`);
      // this.logger.debug(`Signature generated by server: ${generatedSignature}`);
      return;
    }
    ...
    if (webhookData.code !== '00') {
      this.logger.log(
        `Webhook for order ${orderCode} has a non-successful status code: ${webhookData.code}. Skipping.`,
      );
      return;
    }
