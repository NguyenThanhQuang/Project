...
const ManageTrips: React.FC = () => {
  ...
  const handleStatusToggle = (tripId: string) => {
    ...
    showNotification(
      `Trạng thái chuyến xe đã được ${selectedTrip?.status === 'active' ? 'tạm dừng' : 'kích hoạt'}`,
      'success'
    );
    handleMenuClose();
  };
  ...
  const getFilteredTrips = () => {
    switch (tabValue) {
      case 0: return filterTripsByStatus('all');
      case 1: return filterTripsByStatus('active');
      case 2: return filterTripsByStatus('inactive');
  ...
  const getTotalStats = () => {
    ...
    return {
      ...
      totalBookings: filteredTrips.reduce((sum, trip) => sum + trip.bookingCount, 0),
  ...
  return (
    <Container maxWidth="xl" sx={{ py: 4 }}>
            ...
            <Grid size={{ xs: 12, md: 6, lg: 4 }} key={trip.id}>
              <Card elevation={2} sx={{ borderRadius: 3, height: '100%' }}>
                <CardContent>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                    <DirectionsBus sx={{ fontSize: 18, color: 'text.secondary', mr: 1 }} />
                    <Typography variant="body2" color="text.secondary">
                      {trip.vehicleType} - {trip.vehicleNumber} ({trip.seatCount} ghế)
                    </Typography>
                  </Box>
                  ...
                  <Box sx={{ mt: 2 }}>
                    ...
                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5, mt: 0.5 }}>
                      ...
                      {trip.amenities.length > 3 && (
                        <Chip label={`+${trip.amenities.length - 3}`} size="small" variant="outlined" />
                      )}
                    </Box>
                  </Box>
                </CardContent>
      ...
      >
        ...
        <MenuItem onClick={() => selectedTrip && handleStatusToggle(selectedTrip.id)}>
      ...
      >
        ...
        <DialogContent>
          <Typography>
            Bạn có chắc chắn muốn xóa chuyến xe "{selectedTrip?.tripName}"?
