...
import { tripApi, type Trip as ApiTrip } from '../services/api';
...
const TripSearchResults: React.FC = () => {
  ...
  const fetchTrips = async () => {
    ...
    try {
      ...
      const apiTrips = await tripApi.getTrips(query);
      
      // Transform API response to match frontend interface
      const transformedTrips: Trip[] = apiTrips.map((trip: ApiTrip) => ({
        id: trip._id,
        companyName: trip.companyId.name,
        companyLogo: trip.companyId.logo || trip.companyId.name.charAt(0),
        vehicleType: trip.vehicleId.type,
        departureTime: new Date(trip.departureTime).toLocaleTimeString('vi-VN', { 
          hour: '2-digit', 
          minute: '2-digit' 
        }),
        arrivalTime: new Date(trip.arrivalTime).toLocaleTimeString('vi-VN', { 
          hour: '2-digit', 
          minute: '2-digit' 
        }),
        duration: `${trip.route.estimatedDuration}h`,
        fromLocation: trip.route.from,
        toLocation: trip.route.to,
        price: trip.price,
        availableSeats: trip.availableSeats,
        totalSeats: trip.vehicleId.totalSeats,
        rating: 4.5, // Default rating, can be updated when rating system is implemented
