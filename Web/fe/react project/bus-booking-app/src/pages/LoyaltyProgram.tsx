...
const LoyaltyProgram: React.FC = () => {
  ...
  const getVoucherIcon = (category: string) => {
    switch (category) {
      ...
      case 'service': return <CardGiftcard />;
    ...
    <Grid container spacing={4}>
      ...
      <Grid size={{ xs: 12, md: 8 }}>
        <Paper elevation={3} sx={{ borderRadius: 3, p: 4 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
            ...
            <Box>
              <Typography variant="h4" sx={{ fontWeight: 700, color: tierInfo[userPoints.tier].color }}>
                {userPoints.tier}
              </Typography>
              ...
              <Box sx={{ textAlign: 'center' }}>
                <Typography variant="h3" sx={{ fontWeight: 700, color: 'success.main' }}>
                  {tierInfo[userPoints.tier].pointsNeeded ? 
                    (tierInfo[userPoints.tier].pointsNeeded! - userPoints.total).toLocaleString() : '∞'}
                </Typography>
                <Typography variant="body2" color="text.secondary">
                  Điểm để lên hạng
                </Typography>
      ...
      <Grid size={{ xs: 12, md: 4 }}>
        <Grid container spacing={2}>
          <Grid size={{ xs: 12 }}>
            <Card sx={{ borderRadius: 3, background: 'linear-gradient(135deg, #e3f2fd 0%, #bbdefb 100%)' }}>
          ...
          <Grid size={{ xs: 12 }}>
            <Card sx={{ borderRadius: 3, background: 'linear-gradient(135deg, #f3e5f5 0%, #e1bee7 100%)' }}>
              <CardContent sx={{ textAlign: 'center' }}>
            ...
            <CardContent sx={{ p: 3 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                ...
                <Box sx={{ flex: 1 }}>
                  ...
                  <Chip
                    label={voucher.category === 'discount' ? 'Giảm giá' : 
                          voucher.category === 'upgrade' ? 'Nâng cấp' : 'Dịch vụ'}
                    size="small"
                    color="primary"
                    variant="outlined"
                  />
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <Button
                  ...
                  disabled={userPoints.current < voucher.pointsCost || voucher.available === 0}
                  sx={{
        ...
        <Grid size={{ xs: 12, sm: 6 }} key={voucher.id}>
          <Card 
            sx={{ 
              ...
              border: voucher.used ? '2px dashed #ccc' : '2px solid transparent',
            }}
          >
            <CardContent sx={{ p: 3 }}>
          ...
          {selectedVoucher && (
            <Box>
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <Typography variant="body1">
                  Điểm còn lại: <strong>{userPoints.current - selectedVoucher.pointsCost} điểm</strong>
                </Typography>
              </Box>
            </Box>
          )}
