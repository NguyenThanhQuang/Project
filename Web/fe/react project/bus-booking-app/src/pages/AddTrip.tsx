...
const AddTrip: React.FC = () => {
  ...
  const vietnamCities = [
    ...
    'Nha Trang', 'Đà Lạt', 'Vũng Tàu', 'Huế', 'Hội An',
    'Quy Nhon', 'Phan Thiết', 'Mũi Né', 'Phú Quốc', 'Sapa',
    'Hạ Long', 'Ninh Bình', 'Tam Cốc', 'Cao Bằng', 'Buôn Ma Thuột',
    'Pleiku', 'Kon Tum', 'An Giang', 'Cà Mau', 'Rạch Giá',
  ];
  ...
  const handleInputChange = (field: keyof TripData) => 
    (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
      setTripData({ ...tripData, [field]: e.target.value });
    };

  const handleSelectChange = (field: keyof TripData) => 
    (e: any) => {
      setTripData({ ...tripData, [field]: e.target.value });
    };

  const handleDateTimeChange = (field: keyof TripData) => 
    (value: Dayjs | null) => {
      setTripData({ ...tripData, [field]: value });
      
      // Auto calculate duration if both times are set
      if (field === 'arrivalTime' || field === 'departureTime') {
        const depTime = field === 'departureTime' ? value : tripData.departureTime;
  ...
  const updateRouteStop = (stopId: string, field: keyof RouteStop, value: any) => {
  ...
  const removeRouteStop = (stopId: string) => {
    const updatedStops = tripData.routeStops.filter(stop => stop.id !== stopId);
    setTripData({ ...tripData, routeStops: updatedStops });
  };
          ...
          <Grid container spacing={3}>
            ...
            <Grid size={{ xs: 12, md: 6 }}>
              <Autocomplete
                ...
                onChange={(e, value) => setTripData({ ...tripData, fromCity: value || '' })}
                renderInput={(params) => (
            ...
            <Grid size={{ xs: 12, md: 6 }}>
              <Autocomplete
                ...
                onChange={(e, value) => setTripData({ ...tripData, toCity: value || '' })}
                renderInput={(params) => (
                    ...
                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                      {daysOfWeek.map((day) => (
                        <FormControlLabel
                          ...
                          control={
                            <Checkbox
                              checked={tripData.operatingDays.includes(day.value)}
                      ...
                      <Grid container spacing={2}>
                        <Grid size={{ xs: 12, md: 6 }}>
                          <Autocomplete
                            ...
                            options={vietnamCities}
                            value={stop.city}
                            onChange={(e, value) => updateRouteStop(stop.id, 'city', value || '')}
                            renderInput={(params) => (
                              <TextField {...params} label="Thành phố" size="small" />
                            )}
                          />
                        </Grid>
                        <Grid size={{ xs: 12, md: 6 }}>
                        ...
                        <Grid size={{ xs: 12, md: 4 }}>
                          <TimePicker
                            ...
                            onChange={(value) => updateRouteStop(stop.id, 'arrivalTime', value)}
                            sx={{ width: '100%' }}
                        ...
                        <Grid size={{ xs: 12, md: 4 }}>
                          <TimePicker
                            ...
                            onChange={(value) => updateRouteStop(stop.id, 'departureTime', value)}
                            sx={{ width: '100%' }}
                            slotProps={{ textField: { size: 'small' } }}
                          />
                        </Grid>
                        <Grid size={{ xs: 12, md: 4 }}>
                          <TextField
                            ...
                            onChange={(e) => updateRouteStop(stop.id, 'stopDuration', parseInt(e.target.value))}
                            size="small"
              ...
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                {availableAmenities.map((amenity) => (
                  <Chip
                    ...
                    color={tripData.amenities.includes(amenity) ? 'primary' : 'default'}
                    variant={tripData.amenities.includes(amenity) ? 'filled' : 'outlined'}
                  />
                ))}
              </Box>
            </Grid>
              ...
              <Grid container spacing={3}>
                <Grid size={{ xs: 12, md: 6 }}>
                  ...
                  <Typography variant="body2" sx={{ mb: 2 }}>
                    {tripData.vehicleType} - {tripData.vehicleNumber} ({tripData.seatCount} ghế)
                  </Typography>
                  ...
                  <Typography variant="body2" sx={{ mb: 2 }}>
                    {tripData.departureTime?.format('HH:mm')} - {tripData.arrivalTime?.format('HH:mm')} ({tripData.duration})
                  </Typography>
                </Grid>
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
      <Paper elevation={3} sx={{ borderRadius: 3, overflow: 'hidden' }}>
        {/* Header */}
        <Box sx={{ p: 4, background: 'linear-gradient(135deg, #0077be 0%, #004c8b 100%)', color: 'white' }}>
