...
import { Menu, Bell, User, Filter, Search, TrendingUp, X, Home, MapIcon, History, DollarSign, Settings, LogOut, Moon, Sun } from 'lucide-react';
...
const statusConfig = {
  'about-to-depart': { labelKey: 'aboutToDepart', color: 'bg-blue-500 text-white' },
...
export function DriverHome({ driverName, onTripClick, onProfileClick, onNavigationClick, onHistoryClick, onEarningsClick, onSettingsClick, onNotificationsClick, onLogout }: DriverHomeProps) {
  ...
  const mockNotifications = [
    { id: 1, title: t('newTripAssigned'), message: 'TP.HCM → Đà Lạt lúc 05:00', time: `5 ${t('minutesAgo')}`, unread: true },
    { id: 2, title: t('scheduleChanged'), message: 'Chuyến #TX-001 đã được cập nhật', time: `1 ${t('hourAgo')}`, unread: true },
    { id: 3, title: t('tripCompleted'), message: 'Đà Lạt → TP.HCM đã hoàn thành', time: `3 ${t('hoursAgo')}`, unread: false }
  ];
  ...
  const filteredTrips = mockTrips.filter(trip => {
    if (activeTab === 'today') return trip.status === 'about-to-depart' || trip.status === 'running';
  ...
  const totalTripsToday = mockTrips.filter(t => t.status !== 'arrived').length;
          ...
          <div className="fixed left-0 top-0 h-full w-80 bg-white dark:bg-gray-800 shadow-2xl z-50 animate-slide-in-left">
            <div className="p-6">
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center space-x-3">
                  <div className="w-12 h-12 bg-gradient-to-br from-blue-600 to-teal-500 rounded-full flex items-center justify-center text-white">
                    <span style={{ fontSize: 'var(--text-lg)' }}>{driverName.charAt(0).toUpperCase()}</span>
                  </div>
                  <div>
                    <h3 style={{ fontSize: 'var(--text-base)' }} className="text-gray-900 dark:text-white">
                      {driverName}
                    </h3>
                    <p style={{ fontSize: 'var(--text-sm)' }} className="text-gray-500 dark:text-gray-400">
                      {t('driver')}
                    </p>
                  </div>
                </div>
              ...
              <nav className="space-y-2">
                ...
                <button 
                  onClick={() => { setShowSidebar(false); onNavigationClick?.(); }}
                  className="w-full flex items-center space-x-3 px-4 py-3 rounded-xl hover:bg-blue-50 dark:hover:bg-gray-700 transition-colors text-gray-700 dark:text-gray-300"
                >
                ...
                <button 
                  onClick={() => { setShowSidebar(false); onHistoryClick?.(); }}
                  className="w-full flex items-center space-x-3 px-4 py-3 rounded-xl hover:bg-blue-50 dark:hover:bg-gray-700 transition-colors text-gray-700 dark:text-gray-300"
                >
                ...
                <button 
                  onClick={() => { setShowSidebar(false); onEarningsClick?.(); }}
                  className="w-full flex items-center space-x-3 px-4 py-3 rounded-xl hover:bg-blue-50 dark:hover:bg-gray-700 transition-colors text-gray-700 dark:text-gray-300"
                >
                ...
                <button 
                  onClick={() => { setShowSidebar(false); onSettingsClick?.(); }}
                  className="w-full flex items-center space-x-3 px-4 py-3 rounded-xl hover:bg-blue-50 dark:hover:bg-gray-700 transition-colors text-gray-700 dark:text-gray-300"
                >
              ...
              <div className="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700 space-y-3">
                ...
                <button 
                  onClick={() => { setShowSidebar(false); onLogout?.(); }}
          ...
          <div className="fixed top-16 right-4 w-96 bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 z-50 animate-slide-in-right">
            <div className="p-4 border-b border-gray-200 dark:border-gray-700">
              <div className="flex items-center justify-between">
                <h3 style={{ fontSize: 'var(--text-lg)' }} className="text-gray-900 dark:text-white">
                ...
                >
                  <div className="flex items-start space-x-3">
                    ...
                    <div className="flex-1">
                      <h4 style={{ fontSize: 'var(--text-sm)' }} className="text-gray-900 dark:text-white mb-1">
                        {notification.title}
                      </h4>
                      <p style={{ fontSize: 'var(--text-sm)' }} className="text-gray-600 dark:text-gray-400 mb-1">
                        {notification.message}
                      </p>
                      <span style={{ fontSize: 'var(--text-xs)' }} className="text-gray-400 dark:text-gray-500">
                        {notification.time}
                      </span>
                    </div>
                  </div>
        ...
        <div className="px-4 py-4">
          ...
          <div className="flex items-center space-x-3 mb-4">
            ...
            <div>
              ...
              <h2 className="text-gray-900 dark:text-white">{t('driver')} {driverName}</h2>
            </div>
          </div>
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <h3 className="text-gray-900 dark:text-white mb-1">{trip.route}</h3>
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <div className="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-3">
                    <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">{t('vehiclePlate')}</div>
                    <div className="text-gray-900 dark:text-white">{trip.vehiclePlate}</div>
                  </div>
                  <div className="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-3">
                    <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">{t('distance')}</div>
                    <div className="text-gray-900 dark:text-white">{trip.distance}</div>
                  </div>
                </div>

                <div className="flex items-center justify-between text-sm">
                  <div className="text-gray-600 dark:text-gray-400">
                    {t('boarded')}: <span className="text-blue-600 dark:text-blue-400">{trip.passengersBoarded}/{trip.totalPassengers}</span>
                  </div>
                  <div className="text-gray-600 dark:text-gray-400">
                ...
                <div className="mt-3 w-full h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: `${(trip.passengersBoarded / trip.totalPassengers) * 100}%` }}
                  ></div>
                </div>
              </div>
            );
