...
export function BookingManagement() {
  ...
  return (
    <div className="p-6">
      {/* Header */}
      <div className="mb-6">
        <h2 className="text-gray-900 dark:text-white mb-1">{t('bookingManagementTitle')}</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 border border-gray-100 dark:border-gray-700">
          <div className="text-2xl text-gray-900 dark:text-white mb-1">{bookings.length}</div>
          <div className="text-sm text-gray-600 dark:text-gray-400">{t('totalBookingsCount')}</div>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 border border-gray-100 dark:border-gray-700">
          <div className="text-2xl text-green-600 mb-1">{bookings.filter(b => b.status === 'confirmed').length}</div>
          <div className="text-sm text-gray-600 dark:text-gray-400">{t('confirmedBookings')}</div>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 border border-gray-100 dark:border-gray-700">
          <div className="text-2xl text-red-600 mb-1">{bookings.filter(b => b.status === 'cancelled').length}</div>
          <div className="text-sm text-gray-600 dark:text-gray-400">{t('cancelledBookings')}</div>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 border border-gray-100 dark:border-gray-700">
          <div className="text-2xl text-gray-900 dark:text-white mb-1">{formatPrice(totalRevenue)}</div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50 dark:bg-gray-700/50">
              <tr>
                <th className="px-6 py-3 text-left text-xs text-gray-600 dark:text-gray-400 uppercase">{t('ticketCodeColumn')}</th>
                <th className="px-6 py-3 text-left text-xs text-gray-600 dark:text-gray-400 uppercase">{t('passengerColumn')}</th>
                ...
                <th className="px-6 py-3 text-left text-xs text-gray-600 dark:text-gray-400 uppercase">{t('seat')}</th>
                ...
                return (
                  <tr key={booking.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="text-gray-900 dark:text-white">{booking.ticketCode}</div>
                      <div className="text-xs text-gray-500 dark:text-gray-400">{booking.bookingDate}</div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="text-gray-900 dark:text-white">{booking.passengerName}</div>
                      <div className="text-sm text-gray-500 dark:text-gray-400">{booking.phone}</div>
                    </td>
                    <td className="px-6 py-4 text-gray-600 dark:text-gray-400">{booking.route}</td>
                    <td className="px-6 py-4">
                      <div className="text-gray-900 dark:text-white">{booking.time}</div>
                      <div className="text-sm text-gray-500 dark:text-gray-400">{booking.date}</div>
                    </td>
                    <td className="px-6 py-4 text-gray-600 dark:text-gray-400">{booking.seatNumber}</td>
                    <td className="px-6 py-4 text-gray-900 dark:text-white">{formatPrice(booking.price)}</td>
                    <td className="px-6 py-4">
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-2xl max-w-lg w-full p-6">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-gray-900 dark:text-white">{t('bookingDetails')}</h3>
              <button
            ...
            <div className="space-y-4">
              <div className="bg-gray-50 dark:bg-gray-800 rounded-2xl p-4">
                ...
                <div className="text-2xl text-gray-900 dark:text-white">{selectedBooking.ticketCode}</div>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <div className="text-sm text-gray-600 dark:text-gray-400 mb-1">{t('passenger')}</div>
                  <div className="text-gray-900 dark:text-white">{selectedBooking.passengerName}</div>
                </div>
                <div>
                  <div className="text-sm text-gray-600 dark:text-gray-400 mb-1">{t('phoneNumber')}</div>
                  <div className="text-gray-900 dark:text-white">{selectedBooking.phone}</div>
                </div>
              </div>

              <div>
                <div className="text-sm text-gray-600 dark:text-gray-400 mb-1">{t('route')}</div>
                <div className="text-gray-900 dark:text-white">{selectedBooking.route}</div>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <div className="text-sm text-gray-600 dark:text-gray-400 mb-1">{t('departureTime')}</div>
                  <div className="text-gray-900 dark:text-white">{selectedBooking.time} - {selectedBooking.date}</div>
                </div>
                <div>
                  <div className="text-sm text-gray-600 dark:text-gray-400 mb-1">{t('seatNumber')}</div>
                  <div className="text-gray-900 dark:text-white">{selectedBooking.seatNumber}</div>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <div className="text-sm text-gray-600 dark:text-gray-400 mb-1">{t('vehicle')}</div>
                  <div className="text-gray-900 dark:text-white">{selectedBooking.vehiclePlate}</div>
                </div>
                <div>
                  <div className="text-sm text-gray-600 dark:text-gray-400 mb-1">{t('ticketPrice')}</div>
                  <div className="text-blue-600 dark:text-blue-400">{formatPrice(selectedBooking.price)}</div>
                </div>
              </div>
