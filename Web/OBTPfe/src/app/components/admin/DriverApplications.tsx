...
export function DriverApplications() {
  ...
  const [applications, setApplications] = useState<DriverApplication[]>([
    {
      ...
      licenseImageUrl:
        "https://images.unsplash.com/photo-1589395937772-110d6c7bf5e7?w=600&h=400&fit=crop",
    },
    {
      id: "APP002",
      ...
      licenseImageUrl:
        "https://images.unsplash.com/photo-1554224311-bfef2db8f4fb?w=600&h=400&fit=crop",
    },
    {
      id: "APP003",
      fullName: "Lê Văn Nam",
  ...
  const handleApprove = (id: string) => {
    const note = prompt("Thêm ghi chú (tùy chọn):");
    setApplications((apps) =>
      apps.map((app) =>
        app.id === id
          ? { ...app, status: "approved" as const, notes: note || app.notes }
          : app
      )
    );
    setSelectedApp(null);
    alert("Đã phê duyệt đơn đăng ký! Tài xế sẽ nhận được email thông báo.");
  };

  const handleReject = (id: string) => {
    const reason = prompt("Nhập lý do từ chối (sẽ được gửi đến tài xế):");
    if (reason) {
      setApplications((apps) =>
        apps.map((app) =>
          app.id === id
            ? { ...app, status: "rejected" as const, notes: reason }
            : app
        )
      );
      setSelectedApp(null);
      alert("Đã từ chối đơn đăng ký! Email thông báo đã được gửi đến tài xế.");
    }
  };
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `driver-applications-${
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400">
            Quản lý và phê duyệt đơn đăng ký tài xế mới
          </p>
        </div>
        <button
              ...
              {filteredApplications.map((app) => {
                ...
                return (
                  ...
                  >
                    <td className="px-6 py-4">
                      <div className="flex items-center space-x-3">
                        <img
                          src={
                            app.avatarUrl ||
                            `https://ui-avatars.com/api/?name=${app.fullName}&background=random`
                          }
                          alt={app.fullName}
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`inline-flex items-center space-x-1 px-3 py-1 rounded-lg text-xs ${
                ...
                <div className="flex items-center space-x-4">
                  <img
                    src={
                      ...
                      `https://ui-avatars.com/api/?name=${selectedApp.fullName}&background=random`
                    }
              ...
              {selectedApp.licenseImageUrl && (
                <div className="pt-4 border-t border-gray-200 dark:border-gray-700">
                  ...
                  <div
                    ...
                    onClick={() =>
                      setShowImageModal(selectedApp.licenseImageUrl!)
                    }
                  >
                    <img
