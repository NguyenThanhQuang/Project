...
import { Plus, Search, Edit2, Trash2, X, Phone, Mail, Award } from 'lucide-react';
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-gray-900 dark:text-white mb-1">{t('driverManagementTitle')}</h2>
          <p className="text-gray-600 dark:text-gray-400">{t('driverManagementDesc')}</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 border border-gray-100 dark:border-gray-700">
          <div className="text-2xl text-gray-900 dark:text-white mb-1">{drivers.length}</div>
          <div className="text-sm text-gray-600 dark:text-gray-400">{t('totalDriversCount')}</div>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 border border-gray-100 dark:border-gray-700">
          <div className="text-2xl text-green-600 mb-1">{drivers.filter(d => d.status === 'available').length}</div>
          <div className="text-sm text-gray-600 dark:text-gray-400">{t('availableDrivers')}</div>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 border border-gray-100 dark:border-gray-700">
          <div className="text-2xl text-orange-600 mb-1">{drivers.filter(d => d.status === 'busy').length}</div>
          <div className="text-sm text-gray-600 dark:text-gray-400">{t('busyDrivers')}</div>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 border border-gray-100 dark:border-gray-700">
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50 dark:bg-gray-700/50">
              <tr>
                <th className="px-6 py-3 text-left text-xs text-gray-600 dark:text-gray-400 uppercase">{t('driverColumn')}</th>
                <th className="px-6 py-3 text-left text-xs text-gray-600 dark:text-gray-400 uppercase">{t('contactColumn')}</th>
                <th className="px-6 py-3 text-left text-xs text-gray-600 dark:text-gray-400 uppercase">{t('licenseColumn')}</th>
                <th className="px-6 py-3 text-left text-xs text-gray-600 dark:text-gray-400 uppercase">{t('assignedVehicleColumn')}</th>
                  ...
                  <tr key={driver.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <div className="text-gray-900 dark:text-white">{driver.name}</div>
                          <div className="text-xs text-gray-500 dark:text-gray-400">{t('joinedDate')}: {driver.joinDate}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-gray-600 dark:text-gray-400">{driver.licenseNumber}</td>
                    <td className="px-6 py-4 text-gray-600 dark:text-gray-400">{driver.vehicleAssigned}</td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-gray-600 dark:text-gray-400">{driver.totalTrips}</td>
                    <td className="px-6 py-4">
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-gray-900 dark:text-white">{t('addNewDriverTitle')}</h3>
              <button
            ...
            <form className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-gray-700 dark:text-gray-300 mb-2">{t('fullName')}</label>
                  <input
                    type="text"
              ...
              <div>
                <label className="block text-gray-700 dark:text-gray-300 mb-2">{t('licenseNumberLabel')}</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-gray-700 dark:text-gray-300 mb-2">{t('assignedVehicleColumn')}</label>
