...
export function ReviewManagement() {
  ...
  const [reviews, setReviews] = useState<Review[]>([
    {
      ...
      route: 'TP.HCM → Đà Lạt',
      rating: 5,
      comment: 'Dịch vụ tuyệt vời, xe sạch sẽ, tài xế lái xe an toàn. Sẽ tiếp tục ủng hộ!',
      date: '25/12/2024',
      tripDate: '20/12/2024',
      status: 'published',
      likes: 12
  ...
  return (
    <div className="p-6">
      {/* Header */}
      <div className="mb-6">
        <h2 className="text-gray-900 dark:text-white mb-1">{t('reviewManagementTitle')}</h2>
        <p className="text-gray-600 dark:text-gray-400">{t('reviewManagementDesc')}</p>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 border border-gray-100 dark:border-gray-700">
          <div className="text-2xl text-gray-900 dark:text-white mb-1">{reviews.length}</div>
          <div className="text-sm text-gray-600 dark:text-gray-400">{t('totalReviewsStats')}</div>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 border border-gray-100 dark:border-gray-700">
          <div className="flex items-center space-x-1 mb-1">
            <span className="text-2xl text-yellow-500">{avgRating.toFixed(1)}</span>
            <Star className="w-5 h-5 fill-yellow-400 text-yellow-400" />
          </div>
          <div className="text-sm text-gray-600 dark:text-gray-400">{t('averageRating')}</div>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 border border-gray-100 dark:border-gray-700">
          <div className="text-2xl text-green-600 mb-1">{reviews.filter(r => r.status === 'published').length}</div>
          <div className="text-sm text-gray-600 dark:text-gray-400">{t('publishedReviews')}</div>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 border border-gray-100 dark:border-gray-700">
          <div className="text-2xl text-red-600 mb-1">{reviews.filter(r => r.status === 'flagged').length}</div>
          <div className="text-sm text-gray-600 dark:text-gray-400">{t('flaggedReviews')}</div>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 border border-gray-100 dark:border-gray-700">
          <div className="text-2xl text-gray-900 dark:text-white mb-1">{reviews.filter(r => r.rating === 5).length}</div>
          <div className="text-sm text-gray-600 dark:text-gray-400">5 {t('stars')}</div>
        </div>
      </div>
          ...
          >
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-start space-x-4">
                ...
                <div>
                  <div className="text-gray-900 dark:text-white mb-1">{review.userName}</div>
              ...
              <div className="flex items-center space-x-2">
                <div className={`w-2 h-2 ${statusConfig[review.status].color} rounded-full`}></div>
                <span className="text-sm text-gray-600 dark:text-gray-400">{statusConfig[review.status].label}</span>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between pt-4 border-t border-gray-100 dark:border-gray-700">
              <div className="flex items-center space-x-4 text-sm text-gray-600 dark:text-gray-400">
                ...
                <span className="flex items-center space-x-1">
                  ...
                  <span>{review.likes} {t('likesLabel')}</span>
                </span>
              </div>
              <div className="flex items-center space-x-2">
                <button
